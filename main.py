...
async def handle_chat(data: ChatRequest):
    ...
    image_data = supabase.table("image_data")\
        .select("token_name", "image_caption")\
        .eq("unique_id", unique_id)\
        .execute()
        
    wardrobe_data = [
        {"token_name": record["token_name"], "caption": record["image_caption"]} 
    ...
    try:
        ...
        for image_id in image_ids:
            image_response = supabase.table("image_data")\
                .select("image_url")\
                .eq("token_name", image_id)\
                .execute()
        ...
        return ChatResponse(reply=text, images=images)
