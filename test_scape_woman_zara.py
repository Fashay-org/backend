...
class ZaraScraper:
    ...
    def get_women_categories(self):
        try:
            categories = [
                '/en/woman-jackets-l1114.html',           # JACKETS
                '/en/woman-outerwear-l1184.html',         # COATS | PUFFERS
                '/en/woman-blazers-l1055.html',           # BLAZERS | VESTS
                '/en/woman-dresses-l1066.html',           # DRESSES
                '/en/woman-tops-l1322.html',              # TOPS | BODYSUITS
                '/en/woman-knitwear-l1152.html',          # CARDIGANS | SWEATERS
                '/en/woman-shirts-l1217.html',            # SHIRTS | BLOUSES
                '/en/woman-skirts-l1299.html',            # SKIRTS | SHORTS
                '/en/woman-jeans-l1119.html',             # JEANS
                '/en/woman-trousers-l1335.html',          # PANTS
                '/en/woman-tshirts-l1362.html',           # T-SHIRTS
                '/en/woman-sweatshirts-l1320.html',       # SWEATSUITS
                '/en/woman-shoes-l1251.html',             # SHOES
                '/en/woman-bags-l1024.html',              # BAGS
                '/en/woman-accessories-l1003.html',       # ACCESSORIES | JEWELRY
                '/en/woman-lingerie-l4021.html',          # LINGERIE
                '/en/woman-beauty-perfumes-l1415.html',   # FRAGRANCES
                '/en/woman-beauty-makeup-l4414.html'      # BEAUTY
            ]
    ...
    def scrape_product(self, url):
        try:
            ...
            # Also check image carousels and product galleries
            ...
            # Check alternate image sources (product grid and detail views)
            for img in soup.find_all('img', {'class': ['media-image__image', 'media__wrapper--media']}):
                src = img.get('src')
                if src and 'transparent-background' not in src and src not in images:
                    images.append(src)
                srcset = img.get('srcset')
                if srcset:
                    srcs = [s.split(' ')[0] for s in srcset.split(',')]
                    largest = max(srcs, key=lambda x: int(x.split('w=')[-1].split('&')[0]) if 'w=' in x else 0)
                    if largest and 'transparent-background' not in largest and largest not in images:
        ...
        for url in product_urls:
            ...
            if product_data:
                ...
                if len(all_products) % 10 == 0:
                    with open('zara_women_products_progress.json', 'w', encoding='utf-8') as f:
                        json.dump(all_products, f, indent=2, ensure_ascii=False)
                    logger.info(f"Progress saved: {len(all_products)} products")
    
    # Save final results
    if all_products:
        with open('zara_women_products_final.json', 'w', encoding='utf-8') as f:
            json.dump(all_products, f, indent=2, ensure_ascii=False)
        logger.info(f"\nFinished! Saved {len(all_products)} products to zara_women_products_final.json")
    else:
